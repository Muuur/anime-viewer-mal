...
try:
    ...
    sys.stderr.write("No se encontró requests, el programa no funcionará\n")
    exit(1)
try:
    from colorama import init, Fore as f
except ModuleNotFoundError:
    sys.stderr.write("No se encontró colorama, el programa no funcionará\n")
    exit(1)
...
#         return f'Name: {pri}\nSEC: {se}\n'
#     else:
#         sys.stderr.write(f"{f.LIGHTRED_EX}Formato {format} incorrecto, las opciones son table, list y csv\n")
#         exit(1)
        ...
        try:
            if sys.argv[i].lower() in ['-?', '--help']:
                ...
                if sys.argv[i + 1][0] == '-':
                    ...
                    sys.stderr.write(f.LIGHTRED_EX + sys.argv[i + 1] + " no es un número\n")
                    exit(1)
            ...
            elif sys.argv[i].lower() in ['-h', '--home']: # Página principal
                home = True
            elif sys.argv[i].lower() in ['-y', '--yo']: # Página personal
                yo = True
            elif sys.argv[i].lower() in ('-f', '--noopen'): # No apertura
                force = False
            elif sys.argv[i].lower() in ('-o', '--online'): # Modo chrome
                online = True
            else: # Búsqueda
                bus = "+".join(sys.argv[i:])
                break
        except IndexError:
            sys.stderr.write(f"{f.LIGHTRED_EX}Se requiere un argumento extra para {sys.argv[i]}\n")
            exit(1)
        except ValueError:
            sys.stderr.write(f"{f.LIGHTRED_EX}Se esperaba un número para {sys.argv[i]}\n")
            exit(1)
        i += 1
...
for i in chars:
    if i not in ['name',  'english', 'episodes', 'airing', 'genres', 'airstart', 'airend', 'sair', 'fair', 'id', 'type', 'score']:
        sys.stderr.write(f"{f.LIGHTRED_EX}El filtro {i} no existe\n")
        chars.remove(i)
if bool(bus):
    if l != "":
        sys.stderr.write(f"{f.LIGHTRED_EX}No se puede poner un filtro de lista para una búsqueda general\n")
        exit(1)
    if nfilt != ".*":
        sys.stderr.write(f"{f.LIGHTRED_EX}No se puede filtrar por nombre RegEx si no pertenece a tu propia lista\n")
        exit(1)
    if len(chars) < 10:
        sys.stderr.write(f"{f.LIGHTRED_EX}No se puede filtrar el contenido si no pertenece a tu propia lista\n")
        exit(1)
    home, yo = False, False
if rand:
    if home or yo:
        sys.stderr.write(f.LIGHTRED_EX + "No se puede entrar a home o al perfil si hay random\n")
        exit(1)
    if bool(bus) or nfilt != '.*' or len(chars) < 10:
        sys.stderr.write(f.LIGHTRED_EX + "No se puede hacer una búsqueda para un get aleatorio\n")
        exit(1)
if animeid > 0:
    if bool(bus):
        sys.stderr.write(f.LIGHTRED_EX + "No se puede hacer una búsqueda con id")
        exit(1)
...
if online or rand: # Modo online
    if bool(bus) or animeid > 0:
        if animeid > 0:
            try:
                ...
                sys.stderr.write(f.LIGHTRED_EX + "Ha habido un problema intentando conectar con la página\n")
                exit(9005)
            ...
            print(f.LIGHTBLUE_EX + "\nIntroduce que anime quieres ver sus datos: ", end="")
            while True:
                try:
                    ...
                    sys.stderr.write(f.LIGHTRED_EX + "Ha habido un error introduciendo el número, por favor pruebe de nuevo: " + f.LIGHTMAGENTA_EX)
                except NameError:
                    sys.stderr.write(f.LIGHTRED_EX + "Ha habido un error con la captura del enlace\n")
                    exit(9005)
                except r.exceptions.ConnectionError:
                    sys.stderr.write(f.LIGHTRED_EX + "Ha habido un problema intentando conectar con la página\n")
                    exit(9005)
        ...
        print("\b\b ")
        try:
            if force and input(f.BLUE + "\nQuieres abrir la página (Y/N) " + f.LIGHTMAGENTA_EX)[0].upper() in ["S", "Y"]: system(f'start "" "{anime[titles[bus]]["link"]}"' if name == 'nt' else f'xdg-open "{anime[titles[bus]]["link"]}" > /dev/null 2>&1')
        ...
        i = 0
        for j in re.finditer(r'anime_title&quot;:&quot;([^&]+)&', page): # Título
            titles.append(j[1])
            anime.update({j[1]: {}})
        for j in re.finditer(r'&quot;anime_title_eng&quot;:&quot;([^&]*)&', page):
            ...
            i += 1
        i = 0
        for j in re.finditer(r'anime_num_episodes&quot;:(\d+)', page): # Número de episodios
        ...
        for j in re.finditer(r'anime_airing_status&quot;:(\d)', page): # Airing status
            ...
            i += 1
        i = 0
        for j in re.finditer(r'&quot;genres&quot;:\[([^\]]+)\]', page): # Géneros
        ...
        i = 0
        # if l <= 2:
        #     for j in re.finditer(r';start_date_string&quot;:&quot;([^&]+)&quot;,&quot;finish_date_string&quot;:(&quot;)+([^&]+)&', page): # Fechas de inicio y fin
        ...
        chars.remove('airend')
        for j in re.finditer(r';anime_start_date_string&quot;:&quot;([^&]+)&quot;,&quot;anime_end_date_string&quot;:&quot;([^&]+)', page): # Fechas de inicio y fin
        ...
        for j in re.finditer(r'anime_id&quot;:(\d+)', page): # id
            ...
            i += 1
        i = 0
        for j in re.finditer(r'anime_media_type_string&quot;:&quot;(\w+)', page): # id
            anime[titles[i]]['type'] = j[1]
            i += 1
        i = 0
        for j in re.finditer(r'score&quot;:(\d{1,2})', page): # id
            try:
        ...
        if rand:
            ...
            try:
                if force and input(f.BLUE + "\nQuieres abrir la página (Y/N) " + f.LIGHTMAGENTA_EX)[0].upper() in ["S", 'Y']: system(f"start https://myanimelist.net/anime/{anime[titles[rand]]['id']}" if name == 'nt' else f"xdg-open https://myanimelist.net/anime/{anime[titles[rand]]['id']} > /dev/null 2>&1")
            except (EOFError, OSError, IndexError, KeyboardInterrupt):
                exit(0)
        else:
    ...
    if nfilt != ".*" or len(chars) < 10:
        sys.stderr.write(f.LIGHTRED_EX + "No se pueden usar filtros para modo offline\n")
    elif animeid > 0:
